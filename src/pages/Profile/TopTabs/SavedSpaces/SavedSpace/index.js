...
export default function SavedSpace({ route, navigation }) {
  ...
  function calculateAmount() {
    ...
    let totalSeconds = parseInt(realTime.substring(6, 8));
    ...
    if (totalHours < 1) hours = 1;
    else hours = totalHours;

    if (totalMinutes > 0) {
      minutes = parseFloat(
        ((savedSpace.pricePerHour * totalMinutes) / 60).toFixed(2)
      );
    }

    finalTime = parseFloat(hours + minutes);
    ...
    setCalculatedTime(totalHours + ":" + totalMinutes + ":" + totalSeconds);
  }
  ...
  function updateTime() {
    ...
    if (savedAtMinutes > actualMinutes) {
      finalMinutes =
        parseInt(actualMinutes) + Math.abs(savedAtMinutes - actualMinutes);
    }
  ...
  let loadPaymentMethods = paymentMethods.map((item, index) => {
    return <Picker.Item label={item.name} value={item.name} key={index + 1} />;
  });
